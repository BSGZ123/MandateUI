...
public final class TacticsPanel extends ApplicationAdapter {
    ...
    private Table createCharacterPanel(boolean isLeft) {
        ...
        Table topBar = createTopInfoBar(isLeft);
        Table mainContent = createMainContentArea(isLeft);

        container.add(topBar).expandX().fillX().height(40).padTop(5);
        container.row();
        container.add(mainContent).expand().fill();
